...
const getIncludes = () => {
  ...
  const targetFields = [
    'target',
    'target.videos',
    ...combined.map((f) => `target.${f}`),
  ];
  const subjectFields = [
    'subject',
    'subject.videos',
    ...others.map((f) => `subject.${f}`),
    ...combined.map((f) => `subject.${f}`),
  ];
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      const resultsWithLikes = results.map((item) => {
        if (
          item.activities[0].verb === 'post' &&
          likesMap[item.activities[0].subject.id]
        ) {
...
export const getMediaFeed =
  (mediaId, type, cursor, limit = 10) =>
  async (dispatch, getState) => {
    // console.log(mediaId, type);
    // console.log(cursor);
    dispatch({ type: types.GET_MEDIA_FEED, payload: Boolean(cursor) });
    try {
      const results = await Kitsu.one(
        'mediaFeed',
        `${_.capitalize(type)}-${mediaId}`
      ).get({
        page: { limit, cursor },
        filter: {
          // kind: 'posts',
      ...
      mediaFeed.subscribe(async (data) => {
        ...
        if (data.new.length > 0) {
          dispatch({
            type: types.GET_MEDIA_FEED_MORE,
            payload: not,
            meta: not.meta,
          });
        }
        if (data.deleted.length > 0) {
          dispatch({
            type: types.GET_MEDIA_FEED_LESS,
            payload: data.deleted[0],
            meta: not.meta,
          });
        }
...
export const fetchNotifications =
  ...
  async (dispatch, getState) => {
    ...
    dispatch({
      ...
      loadingMoreNotifications: !!cursor,
    });
    try {
      ...
      if (!BasicCache.has(key)) {
        ...
        notificationsStream.subscribe(async (data) => {
          console.log(
            'Notifications stream callback triggered! Fetching more notifications.'
          );
          ...
          if (data.new.length > 0) {
            dispatch({
              type: types.FETCH_NOTIFICATIONS_MORE,
              payload: not,
              meta: not.meta,
            });
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      const response = await fetch(
        `${kitsuConfig.baseUrl}/edge/feeds/notifications/${id}/_${type}`,
        {
          method: 'POST',
          headers: {
