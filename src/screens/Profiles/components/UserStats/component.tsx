...
export class UserStats extends PureComponent {
  ...
  displayGenres(data) {
    const array = Object.keys(data.categories).map((key) => ({
      name: key,
      percent: (data.categories[key] / data.total) * 100,
    }));
    const sorted = array
      ...
      <View style={styles.graphics}>
        ...
        <View style={styles.categoryBreakdown}>
          ...
          <View style={styles.categoryOverlay}>
            ...
            <StyledText bold size="xxsmall" color="grey" numberOfLines={1}>
              {primaryGenre.name}
            </StyledText>
          </View>
        </View>
  ...
  primaryUnit(kind, data) {
    ...
    if (chapters > 15000) {
      return { index: 4, count: chapters };
    }
    if (chapters > 5000) {
      return { index: 3, count: chapters };
    }
    if (chapters > 1000) {
      return { index: 2, count: chapters };
    }
  ...
  percentile(kind, data) {
    const percentile =
      kind === 'anime' ? data.percentiles.time : data.percentiles.units;
  ...
  timeSpent(kind, data) {
    ...
    const timeUnitText =
      kind === 'anime'
        ? primaryUnit.count === 1
          ? primaryUnit.name.slice(0, primaryUnit.name.length - 1)
          : primaryUnit.name
        : null;
    return (
      <View>
            ...
            >
              ...
              {primaryUnit.count.toString().includes('.')
                ...
                : primaryUnit.count
                    ...
                    .replace(/\B(?=(\d{3})+(?!\d))/g, ',')}{' '}
              {kind === 'anime'
                ? `${timeUnitText} spent watching anime.`
                : 'chapters read.'}
            </StyledText>
            <StyledText size="xxsmall" color="grey">
              {data.completed > 0 &&
                `${data.completed.toLocaleString('en-US')} completed. `}
              {percentile > 0 && (
                <React.Fragment>
                  {'More than '}
                  <StyledText
  ...
  render() {
    ...
    if (!amountConsumed || Object.keys(amountConsumed).length === 0) {
      return null;
    }
    if (kind === 'anime' && amountConsumed.statsData.time < 1) {
      return null;
    }
    if (kind === 'manga' && amountConsumed.statsData.units < 1) {
      return null;
    }
    ...
    return (
      <View style={styles.wrap}>
        <View style={styles.main}>
          <StyledText bold size="xsmall" color="dark">{`${capitalize(
            kind
          )} Stats`}</StyledText>
        </View>
        {loading ? (
