...
const DropdownItem = ({ title, onPress }) => (
  <TouchableOpacity activeOpacity={0.8} onPress={() => onPress(title)} style={{ backgroundColor: 'white', flexDirection: 'row', alignItems: 'center', height: dropdownItemHeight }}>
    <Text style={{ fontFamily: 'OpenSans', fontSize: 12, marginHorizontal: 16, color: '#444' }}>{title}</Text>
  </TouchableOpacity>
);
...
class SidebarDropdown extends React.Component {
  ...
  toggle = () => {
    let nextVal = this.state.active ? itemHeight : dropdownItemHeight * this.count;
    Animated.spring(
      this.height,
      {
        toValue: nextVal,
    ...
    return (
      <View>
        <TouchableOpacity activeOpacity={1} onPress={this.toggle} style={styles.sectionListItem}>
          <View>
            <Text style={{ fontFamily: 'OpenSans', fontSize: 10, marginLeft: 8, color: colors.lightGrey }}>{title}</Text>
            <Text style={{ fontFamily: 'OpenSans', fontSize: 12, marginLeft: 8, marginTop: 4, color: '#444' }}>{value}</Text>
          </View>
          <Right>
